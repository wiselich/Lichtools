...
class WLT_OT_SavePie(bpy.types.Operator):
    bl_idname = "wlt.save_pie"
    bl_label = "WLT Save Pie"
...
class WLT_OT_MetaPie(bpy.types.Operator):
    bl_idname = "wlt.meta_pie"
    bl_label = "WLT Meta Pie"
    bl_description = ("Launches the mode-appropriate WLT Pie.")
    bl_options = {'REGISTER'}
    ...
    def invoke(self, context, event):
        ...
        if context.mode == 'OBJECT':
            print("OBJECT MODE")
            bpy.ops.wlt.super_select('INVOKE_DEFAULT', False)
            return {'FINISHED'}


        elif context.mode == 'SCULPT':
            print("SCULPT MODE")
            bpy.ops.wlt.sculpt_pie('INVOKE_DEFAULT', False)
            return {'FINISHED'}
...
class VIEW3D_MT_PIE_orbit_lock(Menu):
    ...
    def draw(self, context):
        layout = self.layout
        pie_mode = context.workspace.WLT.gizmos.cxt_pie_mode
