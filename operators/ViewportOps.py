...
class WLT_OT_ViewAxis(bpy.types.Operator):
    """Select operator wrapper with a pie menu for fancy things"""
    bl_idname = "wlt.view_axis"
    bl_label = "WLT View Axis"
    bl_description = (
                    "Click: Align View\n"
    ...
    override_workspace_settings: BoolProperty(
        name="Override Workspace Settings",
        description=("Use the workspace-defined target object"
                     + "instead of the active object for relative rotation"),
        default=False
    )

    relative_orbit: BoolProperty(
        name="Relative Orbit",
        description=("Use the workspace-defined target object"
                     + "instead of the active object for relative rotation"),
        default=False
    ...
    def invoke(self, context, event):
        ...
        if self.override_workspace_settings:
            bpy.ops.view3d.view_axis('INVOKE_DEFAULT', True, type=axis_flip, align_active=align)
        else:
            if context.workspace.WLT.vp_use_global_target:
                cached_selection = context.selected_objects
                override = bpy.context.copy()
                override['active_object'] = context.workspace.WLT.vp_orbit_target
...
class WLT_OT_set_axis(bpy.types.Operator):
    """Select operator wrapper with a pie menu for fancy things"""
    bl_idname = "wlt.set_axis"
    bl_label = "WLT Set Axis"
