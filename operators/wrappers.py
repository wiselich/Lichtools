...
from ..singletons import WM as workspace_manager
...
class WLT_OT_SaveIncremental(bpy.types.Operator):
    """Pretty much the save function from MachineTools"""
    bl_idname = "wlt.save_incremental"
    bl_label = "WLT Incremental Save"
...
class WLT_OT_BoolToEnum(bpy.types.Operator):
    """Takes a BoolVector and an Enum, makes them play nice"""
    bl_idname = "wlt.bool_to_enum"
    bl_label = "WLT Bool to Enum"
...
class WLT_OT_CycleEnum(bpy.types.Operator):
    """Wrapper for context_cycle_enum that updates the 3d viewport"""
    bl_idname = "wlt.cycle_enum"
    bl_label = "WLT Cycle Enum"
...
class WLT_OT_SetEnum(bpy.types.Operator):
    """Wrapper for context_set_enum that updates the 3d viewport"""
    bl_idname = "wlt.set_enum"
    bl_label = "WLT Set Enum and Update"
...
class WLT_OT_ContextOp(bpy.types.Operator):
    """Root Operator for checking modifer key status on operator invocation"""
    bl_idname = "wlt.context_op"
    bl_label = "WLT Context Operator"
    ...
    def attempt(self, context, _op, args, props, db=False):
        ...
        if not _op.startswith("WLT_noprop."):
        ...
        # try:
        if _op.startswith("WLT_noprop."): # Not sure why this was an issue before.
...
class WLT_OT_TogglePhotoMode(bpy.types.Operator):
    """Wrapper for context_cycle_enum that updates the 3d viewport"""
    bl_idname = "wlt.toggle_photo_mode"
    bl_label = "WLT Toggle Photo Mode"
    ...
    def invoke(self, context, event):
        ...
        if hasattr(context.space_data, 'overlay'):
            ...
            self.report({'WARNING'}, "3D View Only; Aborting")
            bpy.context.area.tag_redraw()
            return {'CANCELLED'}
...
class WLT_OT_EnhancedSelect(bpy.types.Operator):
    """Select operator wrapper with a pie menu for fancy things"""
    bl_idname = "wlt.super_select"
    bl_label = "WLT Super Select"
    ...
    def invoke(self, context, event):
        ...
        def draw_pie(self, context):
            ...
            if context.mode == 'OBJECT':
                ...
                # TOP
                op = pie.operator("wlt.obj_display", text="Toggle Display")
                op.location = m_loc
                op.do_toggle = True
                op.mode = 'BOUNDS'
...
class WLT_OT_EnhancedTag(bpy.types.Operator):
    """Shortest Path Pick operator wrapper with a pie menu for fancy things"""
    bl_idname = "wlt.super_tag"
    bl_label = "WLT Super Tag"
    bl_description = """Fancy Things"""
...
class WLT_OT_GroupSelect(bpy.types.Operator):
    """Object-Mode Selection That Actually Makes Sense"""
    bl_idname = "wlt.group_select"
    bl_label = "WLT Group Select"
...
class WLT_OT_AddToMode(bpy.types.Operator):
    """Wrapper for editmode_toggle that uses overrides to enable mode expansion"""
    bl_idname = "wlt.add_to_mode"
    bl_label = "WLT Add Object to Mode"
...
class WLT_OT_MoveCursor(bpy.types.Operator):
    """Wrapper for various 3D Cursor Operators, with context overrides"""
    bl_idname = "wlt.move_cursor"
    bl_label = "WLT Move Cursor"
...
class WLT_OT_FastSnap(bpy.types.Operator):
    """Wrapper for various transforms, which toggles snapping and pivot setting"""
    bl_idname = "wlt.fast_snap"
    bl_label = "WLT Fast Snap"
...
class WLT_OT_context_modal_mouse(bpy.types.Operator):
    """Adjust arbitrary values with mouse input"""
    bl_idname = "wlt.context_modal_mouse"
    bl_label = "WLT Context Modal Mouse"
    bl_options = {'GRAB_CURSOR', 'BLOCKING', 'UNDO', 'INTERNAL'}

    initial_zoom: FloatProperty(
        description="Starting Zoom Value",
        default=0.0,
        options={'HIDDEN'}
    )
...
class WLT_OT_drop_tool(bpy.types.Operator):
    """Changes the active tool to a contextually-appropriate 'default tool'"""
    bl_idname = "wlt.drop_tool"
    bl_label = "WLT Drop Tool"
    # bl_options = {'GRAB_CURSOR', 'BLOCKING', 'UNDO', 'INTERNAL'}
    ...
    def invoke(self, context, event):
        ...
        if mode in base_modes:
            ...
            self.report({'WARNING'}, "WLT_OT_drop_tool: Something Broke")

        return{'FINISHED'}


class WLT_OT_Set_Custom_Popover(bpy.types.Operator):
    """Let's the user set which panel will be displayed by the associated popover button"""
    bl_idname = "wlt.set_custom_pop"
    bl_label = "WLT Set Custom Popover"
    ...
    def execute(self, context):
        button = self.button
        WLT_workspace = context.workspace.WLT
        ...
        if is_valid:
            ...
            self.report({'WARNING'}, self.panel_name + " is Invalid or Missing")
            return {'CANCELLED'}

        if button == 1:
            WLT_workspace.popovers.popover_1 = self.panel_name
        elif button == 2:
            WLT_workspace.popovers.popover_2 = self.panel_name
        elif button == 3:
            WLT_workspace.popovers.popover_3 = self.panel_name
...
class WLT_OT_Clear_Custom_Popover(bpy.types.Operator):
    """Let's the user clear the panel of the associated popover button"""
    bl_idname = "wlt.clear_custom_pop"
    bl_label = "WLT Clear Custom Popover"
    bl_options = {'BLOCKING', 'UNDO', 'INTERNAL', 'REGISTER'}
    ...
    def execute(self, context):
        ...
        if button == 1:
            WLT_workspace.popovers.popover_1 = "BATMAN"
        elif button == 2:
            WLT_workspace.popovers.popover_2 = "BATMAN"
        elif button == 3:
            WLT_workspace.popovers.popover_3 = "BATMAN"
...
class WLT_OT_Frame_Object(bpy.types.Operator):
    """Forces the Frame Selected operator to frame the object in edit mode"""
    bl_idname = "wlt.frame_object"
    bl_label = "WLT Frame Object"
...
class WLT_OT_Select_Object(bpy.types.Operator):
    """A UI-Only Operator for setting the active object selection"""
    bl_idname = "wlt.object_select"
    bl_label = "WLT Frame Object"
    bl_options = {'BLOCKING', 'UNDO', 'REGISTER'}
...
class WLT_OT_Nuke_Panel(bpy.types.Operator):
    """A UI-Only Operator for setting the active object selection"""
    bl_idname = "wlt.nuke_panel"
    bl_label = "WLT Nuke Panel"
...
class WLT_OT_Set_Object_Display(bpy.types.Operator):
    """A Macro that selects an object, sets its display mode, then deselects it"""
    bl_idname = "wlt.obj_display"
    bl_label = "WLT Set Object Display Mode"
    bl_description = "Takes a BoolVector and an Enum, makes them play nice"
...
class WLT_OT_New_File(bpy.types.Operator):
    """Uses the Workspace system"""
    bl_idname = "wlt.new_file"
    bl_label = "WLT New File"
    bl_options = {'BLOCKING', 'UNDO', 'REGISTER'}

    def invoke(self, context, event):
        ws = workspace_manager.active_config

        if ws:
            ws_settings = ws.get("workspace.settings")
            template_path = ws_settings.get("new_file_base")
...
class WLT_OT_MultiPop(bpy.types.Operator):
    """Opens a different popover depending on the modifier key"""
    bl_idname = "wlt.multi_pop"
    bl_label = "WLT Popover"
    bl_description = "Jesus on a pogo stick"

    def_panel: StringProperty(
        name="Default Panel",
        description="",
        default=""
    )

    ctrl_panel: StringProperty(
        name="Ctrl Panel",
        description="",
        default=""
    )

    alt_panel: StringProperty(
        name="Alt Panel",
        description="",
        default=""
    )

    shift_panel: StringProperty(
        name="Shift Panel",
        description="",
        default=""
    )
    ...
    def description(cls, context, properties):
        tooltip = "Default: " + properties.def_panel

        if properties.ctrl_panel:
            tooltip = tooltip + " | Ctrl: " + properties.ctrl_panel

        if properties.alt_panel:
            tooltip = tooltip + " | Alt: " + properties.alt_panel

        if properties.shift_panel:
            tooltip = tooltip + " | Shift: " + properties.shift_panel
    ...
    def invoke(self, context, event):
        name = None
        if event.ctrl:
            name = self.ctrl_panel
        elif event.alt:
            name = self.alt_panel
        elif event.shift:
            name = self.shift_panel
        else:
            name = self.def_panel

        if name:
            bpy.ops.wm.call_panel(name=name, keep_open=False)
            return {'FINISHED'}
        else:
            return {'CANCELLED'}
...
OUT = [
    ...
    WLT_OT_ContextOp,
    WLT_OT_BoolToEnum,
    WLT_OT_EnhancedSelect,
    WLT_OT_EnhancedTag,
    WLT_OT_GroupSelect,
    WLT_OT_AddToMode,
    WLT_OT_MoveCursor,
    WLT_OT_FastSnap,
    WLT_OT_context_modal_mouse,
    WLT_OT_drop_tool,
    WLT_OT_Set_Custom_Popover,
    WLT_OT_Clear_Custom_Popover,
    WLT_OT_TogglePhotoMode,
    WLT_OT_Frame_Object,
    WLT_OT_Select_Object,
    WLT_OT_Nuke_Panel,
    WLT_OT_Set_Object_Display,
    WLT_OT_SaveIncremental,
    WLT_OT_New_File,
    WLT_OT_MultiPop,
]