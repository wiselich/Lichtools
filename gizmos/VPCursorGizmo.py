...
def giz_color():
    selector = bpy.context.workspace.WLT.gizmos.ctx_pie_mode
    color = 0.5, 0.5, 0.5
    if selector ==  'VERTEX':
        color = 0.95, 0.95, 0.5
    elif selector == 'EDGE':
...
class ATVPCursorGizmo(GizmoGroup):
    ...
    def setup_keymap(self, keys):
        ...
        if not keys.keymaps.find(name="WLT Gizmo", space_type="VIEW_3D"):
            print("No Gizmo Keymap - Making a New One")
            km = keys.keymaps.new(name='WLT Gizmo', space_type="VIEW_3D")
            ...
            kmi.properties.name = "VIEW3D_MT_WLT_cursor_pie"
        else:
            km = keys.keymaps.find(name="WLT Gizmo", space_type="VIEW_3D")
            km.restore_to_default()
            # print("WLT Gizmo Keymap Found: " + str(len(km.keymap_items)) + " Entries")
