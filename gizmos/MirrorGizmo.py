...
def set_pivot(axis):
    ...
    axes = [
        mathutils.Quaternion((0.0, 1.0, 0.0), math.radians(90.0)), # X axis — 'Positive' (blender's axis order is screwy)
    ...
    if obj:
        if bpy.context.workspace.WLT.modal_state.mm_use_cursor:
...
def set_op(arrow, axis):
    ...
    props = arrow.target_set_operator("wlt.quick_symmetry")
    props.mirror_axis = axes[axis]
    props.do_modal = False
    return axes[axis]


class WLT_MirrorGizmoGroup(GizmoGroup):
    bl_idname = "WLT_mirror_gizmo_group"
    bl_label = "WLT Mirror Gizmo"
    ...
    def poll(cls, context):
        if bpy.context.workspace.WLT.modal_state.mirror_modal:
            if isModalRunning():
                return True
